...
{
int i = 0 ;
    SDL_Init(SDL_INIT_VIDEO); //initialiser
    ...
    ecran=SDL_SetVideoMode(800, 606, 32, SDL_HWSURFACE);
...
Mix_Music *musique;//pointeur musique
    ...
    Mix_Chunk *MB;// pointeur musique boutton
    MB = Mix_LoadWAV("button.wav");// clique boutton 
    SDL_Event event;// evenement 
    int continuer = 1;

    SDL_BlitSurface(imageDeFond, NULL, ecran, &positionFond);
//affichage
SDL_BlitSurface(imageDeFond, NULL, ecran, &positionFond);//affichage
...
SDL_BlitSurface(image8,NULL, ecran, &positionim);
 ...
 setrects_right(SDL_Rect clip[]);
 setrects_left(SDL_Rect clip[]);
 setrects_crouchright(SDL_Rect clip[]);
 setrects_crouchleft(SDL_Rect clip[]);
 animation(acteur *acteur);
SDL_Flip(ecran);
                }
...
Mix_PlayChannel(1, MB, 0);
 ...
 SDL_Flip(ecran);
    ...
    Mix_CloseAudio();// arreter la musique 
    SDL_Quit(); //quitter
